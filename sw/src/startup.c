...
#define _STACK_SIZE 0x400 // 1k
// Add stack as symbol to simplify debugging
static uint32_t _stack[_STACK_SIZE] __attribute__ ((aligned(8), used, section(".stack")));
 ...
 * Alias to a dummy handler unless otherwise declared.
 */
...
void Reset_Handler(void) {
  ...
  /*
  uint32_t *adr_stext = &_stext;
  uint32_t *adr_etext = &_etext;
  uint32_t *adr_sdata = &_sdata;
  uint32_t *adr_edata = &_edata;
  uint32_t *adr_sbss = &_sbss;
  uint32_t *adr_ebss = &_ebss;
  */
...
//#if MYCPU==M4
  /* Enable FPU */
//  *((uint32_t*) 0xE000ED88) |= ((3UL << 10*2)|(3UL << 11*2));  /* set CP10 and CP11 Full Access */
...
//uint8_t pStack[1024] __attribute__ ((section(".pstack"))) = {0};
//uint8_t mStack[1024] __attribute__ ((section(".mstack"))) = {0};
